...
app.get("/test-tp1", async (req, res) => {
  const { pair, direction, entry, tp1, tp2, sl } = {
    pair: "BTCUSDT", direction: "LONG",
    entry: "78500.00", tp1: "79200.00", tp2: "79900.00", sl: "77800.00",
  };

  const publicCaption =
`🎯 TP1 HIT — ${pair}

Our ${direction} signal played out!
...
Xenon Alpha Pro ⚡`;
  ...
  try {
    ...
    console.log("Test TP1 sent (mode:", postMode, ")");
    res.json({ ok: true, mode: postMode, caption: publicCaption });
  } catch (err) {
    console.error("Test TP1 failed:", err.message);
    res.json({ ok: false, error: err.message });
  }
});

app.get("/test-tp2", async (req, res) => {
  const { pair, direction, entry, tp1, tp2 } = {
    pair: "BTCUSDT", direction: "LONG",
    entry: "78500.00", tp1: "79200.00", tp2: "79900.00",
  };
...
This is why we trade with Xenon Alpha Pro 🔥⚡`;
  ...
  try {
    ...
    console.log("Test TP2 sent (mode:", postMode, ")");
    res.json({ ok: true, mode: postMode, caption: publicCaption });
  } catch (err) {
    console.error("Test TP2 failed:", err.message);
    res.json({ ok: false, error: err.message });
  }
});
